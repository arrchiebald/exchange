...
def start(message):
    ...
    text = '*Добрый день*, вся хуйня aklsdmalkd'
    ...
    sellect_action(message)

# Выбор действия после старта
def sellect_action(message):
...
def requisites_usdt(message):
    user_history = session.query(ApplicationsBuy).filter(ApplicationsBuy.user_id==message.chat.id)
    markup = types.InlineKeyboardMarkup()
